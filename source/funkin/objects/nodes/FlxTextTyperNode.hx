...
/**
 * Typer Utility Node to make typed text simpler.
 * 
 * Ripped from https://github.com/Geokureli/flixel-tests/blob/main/Source/flixel/addons/text/FlxTextTyper.hx
 * Use case:
 * ```haxe
 * // mainText will type out "hello"
 ...
 * ```
 */
class FlxTextTyperNode extends FlxBasic
{
	/**
	...
	override function update(elapsed:Float)
	{
		...
		if (changed)
		{
			...
			if (cursorShowing) text += cursorCharacter;
	...
	public function startTyping(text:String, clear = true)
	{
		...
		if (clear || (typedText != "" && finalText.indexOf(typedText) != 0)) typedText = "";
	...
	public function startAppending(text:String, skipCurrent = true)
	{
		...
		if (skipCurrent && (state == TYPING || state == ERASING)) skip();
	...
	{
		if (textOverride != null) typedText = textOverride;
	...
	function updateTyping(elapsed:Float):Bool
	{
		...
		if (changed) onCharsType.dispatch();
	...
	{
		if (charDelayScaler.exists(char)) return delay.scale(charDelayScaler[char]).random();
	...
	function updateErasing(elapsed:Float):Bool
	{
		...
		if (changed) onCharsErase.dispatch();
...
@:using(funkin.objects.nodes.FlxTextTyperNode.FlxTypeDelayTools)
enum FlxTypeDelay
{
	CONST(delay:Float);
